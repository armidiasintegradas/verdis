...
begin
  ...
  if not app_private.has_permission(v_user_id,v_movement.tenant_id,v_movement.organization_id,v_movement.unit_id,'sale.create') or not app_private.has_permission(v_user_id,v_movement.tenant_id,v_movement.organization_id,v_movement.unit_id,'movement.create') then raise exception 'sale draft update is not authorized'; end if;
  select c.tenant_id,c.organization_id into v_buyer_tenant,v_buyer_org from public.counterparties c where c.id=p_buyer_counterparty_id;
  if v_buyer_tenant is null or v_buyer_tenant<>v_movement.tenant_id or v_buyer_org<>v_movement.organization_id then raise exception 'buyer counterparty must belong to seller organization and tenant'; end if;
  select m.tenant_id into v_material_tenant from public.materials m where m.id=p_material_id;
  if v_material_tenant is null or v_material_tenant<>v_movement.tenant_id then raise exception 'sale material must belong to sale tenant'; end if;
  ...
  update public.movements set material_id=p_material_id,quantity_kg=p_quantity_kg,destination_counterparty_id=p_buyer_counterparty_id,occurred_at=p_sold_at where id=p_movement_id;
  update public.sales set buyer_counterparty_id=p_buyer_counterparty_id,material_id=p_material_id,quantity_kg=p_quantity_kg,unit_price=p_unit_price,sold_at=p_sold_at where id=v_sale.id returning total_amount into v_total;
...
create or replace function public.confirm_sale_m1(p_movement_id uuid,p_decision text,p_evidence_id uuid default null,p_reason text default null)
returns table(movement_id uuid,sale_id uuid,adopted_quantity_kg numeric,adopted_unit_price numeric,adopted_total_amount numeric,previous_stock_kg numeric,new_stock_kg numeric)
language plpgsql security definer set search_path=public,pg_temp as $$
declare v_user_id uuid:=auth.uid(); v_movement public.movements; v_sale public.sales; v_evidence public.evidences; v_extracted jsonb; v_document_quantity numeric; v_document_unit_price numeric; v_document_total numeric; v_previous_stock numeric; v_new_stock numeric; v_reason text:=nullif(btrim(coalesce(p_reason,'')),''); v_final_price numeric; v_final_total numeric;
begin
 if v_user_id is null then raise exception 'authenticated user is required'; end if;
 select m.* into v_movement from public.movements m where m.id=p_movement_id for update;
 if v_movement.id is null then raise exception 'sale not found'; end if;
 ...
 if p_decision='registered_only' then if p_evidence_id is not null then raise exception 'registered_only must not include evidence'; end if;
 else
  ...
  begin v_document_quantity:=nullif(v_extracted->>'quantity_kg','')::numeric; v_document_unit_price:=nullif(v_extracted->>'unit_price','')::numeric; v_document_total:=nullif(v_extracted->>'total_amount','')::numeric; exception when invalid_text_representation then raise exception 'processed sale evidence values are invalid'; end;
  ...
  insert into public.validations(movement_id,evidence_id,document_id,validation_type,status,automated,actor_user_id,rule_code,reason) values(p_movement_id,v_evidence.id,v_evidence.document_id,'operator_resolution','accepted',false,v_user_id,case p_decision when 'use_document' then 'SALE_USE_DOCUMENT_PRICE' else 'SALE_KEEP_REGISTERED_PRICE' end,v_reason);
  if p_decision='use_document' then update public.sales set unit_price=v_document_unit_price where id=v_sale.id; v_sale.unit_price:=v_document_unit_price; end if;
 end if;
