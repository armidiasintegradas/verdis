...
export function ReceiptFlowPage({ movementId }: ReceiptFlowPageProps) {
  ...
  return <div className="v-page-grid" data-testid="receipt-flow-page">
    ...
    {effectiveStep === 'comprovacao' ? <section className="v-receipt-panel">
      ...
      {uploadState.kind === 'uploading' ? <div className="v-receipt-document-card"><div><strong>{uploadState.file.name}</strong><span>Enviando...</span></div><StatusBadge tone="info">Enviando</StatusBadge></div> : null}
      {uploadState.kind === 'uploaded' ? <div className="v-receipt-document-card"><div><strong>{uploadState.filename}</strong><span>Documento enviado</span></div><StatusBadge tone="info">Processando</StatusBadge></div> : null}
